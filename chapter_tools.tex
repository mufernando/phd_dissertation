...
We computed three statistics used for finding selective sweeps along the simulated regions: (i) nucleotide diversity, (ii) the probability of a sweep using a machine learning sweep classifier called diploSHIC, and (iii) the composite likelihood ratio (CLR) from the model described in. %TODO:add citations
...
diploSHIC has good power overall, reaching close to $100\%$ at many locations with YRI demography, but it faulters at around $15\%$ of the locations where it dips below $75\%$.
...
We found that sweeps placed in higher recombination region are generally harder to detect \plcref{fig:power_rec}.
...
These findings implicate that previous scans have only found sweeps where they can be found.
